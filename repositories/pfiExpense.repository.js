 ...
 */
/**
 * What an expense costs the company, in naira.
 *
 * The `_ngn` columns rather than the raw ones, because `amount` is denominated
 * in the invoice's own currency (db/migrations/0026) and summing it across a
 * mixed set adds dollars to naira as though they were the same unit. The naira
 * columns are generated by the database from the amount and its rate, so they
 * cannot be stale or disagree with what they were derived from.
 *
 * Paid rows use what actually cleared, at the rate on the day it cleared —
 * which is why a foreign invoice's cost is not knowable until it is paid.
 */
