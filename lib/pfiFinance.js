...
function explainFinancials(pfi, f) {
  ...
  return [
    ...
    },
    {
      ...
      meaning:
        f.remaining < 0
          ? `OVERSOLD. Paid orders on this batch come to ${QTY(f.sold, unit)} against a tank of ${QTY(f.tankQtyLitres, unit)} — ${QTY(Math.abs(f.remaining), unit)} more than it ever held. Either the stock was drawn from another batch at the same depot, or orders belonging elsewhere were booked here. This is not leftover stock.`
          : f.awaitingPayment > 0
