...
/**
 * denyPfiScoped, except for someone who has also been given filling stations.
 *
 * Filling stations are an area with no PFI link, so a PFI-confined person is
 * kept out of it — unless somebody deliberately assigned them stations too.
 * That assignment is the explicit "this is yours" the blanket refusal exists
 * to wait for, and inside the area they see only those stations
 * (lib/stationScope.js).
 */
const denyPfiScopedUnlessStations = (req, res, next) => {
  if ((req.user?.scope?.fillingStationIds || []).length) return next();
  return denyPfiScoped(req, res, next);
};
